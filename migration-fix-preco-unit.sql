-- Fix: adicionar coluna preco_unit se não existir em maint_servico_materiais
-- Corre no Supabase SQL Editor

ALTER TABLE maint_servico_materiais
  ADD COLUMN IF NOT EXISTS preco_unit NUMERIC(10,2) NOT NULL DEFAULT 0;

-- Re-criar a função de relatórios (idempotente)
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'por_estado', (
      SELECT COALESCE(
        jsonb_agg(jsonb_build_object('estado', estado, 'total', total) ORDER BY total DESC),
        '[]'::jsonb
      )
      FROM (
    ...
    'por_equipa', (
      SELECT COALESCE(
        jsonb_agg(jsonb_build_object('nome', nome, 'total', total) ORDER BY total DESC),
        '[]'::jsonb
      )
      FROM (
    ...
    'materiais', (
      ...
      FROM (
        SELECT
          ...
          SUM(sm.quantidade * COALESCE(sm.preco_unit, 0)) AS custo_total
